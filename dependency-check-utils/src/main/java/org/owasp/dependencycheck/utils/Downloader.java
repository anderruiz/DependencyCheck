...
public final class Downloader {
		...
		if ("file".equalsIgnoreCase(url.getProtocol())) {
			...
			try {
				...
				while ((status == HttpURLConnection.HTTP_MOVED_TEMP || status == HttpURLConnection.HTTP_MOVED_PERM
						|| status == HttpURLConnection.HTTP_SEE_OTHER) && MAX_REDIRECT_ATTEMPTS > redirectCount++) {
			...
			try (OutputStream out = new FileOutputStream(outputPath); BufferedOutputStream writer = new BufferedOutputStream(out)) {
				...
				final String msg = format("Error saving '%s' to file '%s'%nConnection Timeout: %d%nEncoding: %s%n", url.toString(),
						outputPath.getAbsolutePath(), conn.getConnectTimeout(), encoding);
				throw new DownloadFailedException(msg, ex);
			}
			catch (Exception ex) {
	...
	/**
	 * Makes an HTTP Head request to retrieve the last modified date of the given URL. If the file:// protocol is specified, then the
	 * lastTimestamp of the file is returned.
	 *
	...
	/**
	 * Analyzes the IOException, logs the appropriate information for debugging purposes, and then throws a DownloadFailedException that
	 * wraps the IO Exception for common IO Exceptions. This is to provide additional details to assist in resolution of the exception.
	 *
	...
	protected static synchronized void checkForCommonExceptionTypes(final IOException ex) throws DownloadFailedException {
		...
		while (cause != null) {
			...
			if (cause instanceof InvalidAlgorithmParameterException) {
				...
				LOGGER.info("Java Info:\njavax.net.ssl.keyStore='{}'\njava.version='{}'\njava.vendor='{}'", keystore, version, vendor);
