...
public abstract class BaseDependencyCheckMojo extends AbstractMojo implements MavenReport {
	...
	/**
	 * The entry point towards a Maven version independent way of resolving artifacts (handles both Maven 3.0 Sonatype and Maven 3.1+
	 * eclipse Aether implementations).
	 */
	@SuppressWarnings("CanBeFinal")
	@Component
	...
	/**
	 * Specifies if the build should be failed if a CVSS score above a specified level is identified. The default is 11 which means since
	 * the CVSS scores are 0-10, by default the build will never fail.
	 */
	@SuppressWarnings("CanBeFinal")
	@Parameter(property = "failBuildOnCVSS", defaultValue = "11", required = true)
	private final float failBuildOnCVSS = 11;
	...
	private final boolean failBuildOnAnyVulnerability = false;
	...
	/**
	 * The report format to be generated (HTML, XML, VULN, ALL). This configuration option has no affect if using this within the Site
	...
	private final boolean showSummary = true;
	...
	private final boolean skipTestScope = true;
	...
	private final boolean skipRuntimeScope = false;
	...
	private final boolean skipProvidedScope = false;
	...
	private final boolean skipSystemScope = false;
	...
	@Deprecated
	private final String externalReport = null;
	...
	/**
	 * An array of <code>fileSet</code>s that specify additional files and/or directories (from the basedir) to analyze as part of the scan.
	 * If not specified, defaults to Maven conventions of: src/main/resources, src/main/filters, and src/main/webapp
	 */
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		if (skip) {
			...
			validateAggregate();
			project.setContextValue(getOutputDirectoryContextKey(), outputDirectory);
			runCheck();
		}
	}

	/**
	 * Checks if the aggregate configuration parameter has been set to true. If it has a MojoExecutionException is thrown because the
	 * aggregate configuration parameter is no longer supported.
	 *
	...
	@Override
	@Deprecated
	public final void generate(@SuppressWarnings("deprecation") final org.codehaus.doxia.sink.Sink sink, final Locale locale)
	...
	protected File getCorrectOutputDirectory() throws MojoExecutionException {
		return getCorrectOutputDirectory(project);
	}
	 ...
	 * @return the directory to write the report(s)
	 */
	protected File getCorrectOutputDirectory(final MavenProject current) {
	 ...
	 * @return a collection of exceptions that may have occurred while resolving and scanning the dependencies
	 */
	protected ExceptionCollection scanArtifacts(final MavenProject project, final Engine engine) {
		try {
	...
	/**
	 * Resolves the projects artifacts using Aether and scans the resulting dependencies.
	 *
	 ...
	 * @return a collection of exceptions that may have occurred while resolving and scanning the dependencies
	 */
	private ExceptionCollection collectDependencies(final Engine engine, final MavenProject project, final List<DependencyNode> nodes,
			...
			try {
				...
				if (org.apache.maven.artifact.Artifact.SCOPE_SYSTEM.equals(dependencyNode.getArtifact().getScope())) {
					...
					if (!isResolved) {
						...
						exCol.addException(new DependencyNotFoundException(
				...
				if (isResolved && artifactFile != null) {
					...
					if (deps != null) {
						if (deps.size() == 1) {
							...
							final String msg = String.format(
									"More than 1 dependency was identified in first pass scan of '%s' in project %s",
					...
					final String msg = String.format("Unable to resolve '%s' in project %s", dependencyNode.getArtifact().getId(),
							project.getName());
					getLog().debug(msg);
					if (exCol == null) {
						exCol = new ExceptionCollection();
	 ...
	 */
	private static boolean artifactsMatch(final org.apache.maven.model.Dependency d, final Artifact a) {
		return isEqualOrNull(a.getArtifactId(), d.getArtifactId()) && isEqualOrNull(a.getGroupId(), d.getGroupId())
	 ...
	 * @return true if the strings are equal or if they are both null; otherwise false.
	 */
	...
	/**
	 * @return Returns a new ProjectBuildingRequest populated from the current session and the current project remote repositories, used to
	 * resolve artifacts.
	 */
	...
	protected void runCheck() throws MojoExecutionException, MojoFailureException {
		...
		try {
			...
			if (exCol == null || !exCol.isFatal()) {
				...
				File outputDir = getCorrectOutputDirectory(getProject());
				if (outputDir == null) {
					...
					outputDir = new File(getProject().getBuild().getDirectory());
				}
				try {
					final MavenProject p = getProject();
	 ...
	 */
	private ExceptionCollection handleAnalysisExceptions(final ExceptionCollection currentEx, final ExceptionCollection newEx)
			throws MojoExecutionException {
	...
	public String getOutputName() {
		...
		else if ("VULN".equalsIgnoreCase(format)) {
	...
	/**
	 * Initializes a new <code>Engine</code> that can be used for scanning. This method should only be called in a try-with-resources to
	 * ensure that the engine is properly closed.
	 *
	...
	/**
	 * Takes the properties supplied and updates the dependency-check settings. Additionally, this sets the system properties required to
	 * change the proxy url, port, and connection timeout.
	 */
	protected void populateSettings() {
		...
		if (proxyUrl != null && !proxyUrl.isEmpty()) {
			getLog().warn(
					"Deprecated configuration detected, proxyUrl will be ignored; use the maven settings to configure the proxy instead");
		}
				...
				try {
					...
					if (ex.getCause() instanceof FileNotFoundException
						...
						if (tmp.startsWith("{") && tmp.endsWith("}")) {
							getLog().error(
									String.format("Unable to decrypt the server password for server id '%s' in settings.xml%n\tCause: %s",
	...
	/**
	 * Returns a reference to the current project. This method is used instead of auto-binding the project via component annotation in
	 * concrete implementations of this. If the child has a <code>@Component MavenProject project;</code> defined then the abstract class
	 * (i.e. this class) will not have access to the current project (just the way Maven works with the binding).
	 *
	 ...
	 */
	protected void checkForFailure(final Dependency[] dependencies) throws MojoFailureException {
		...
		if (ids.length() > 0) {
			...
			if (showSummary) {
				if (failBuildOnAnyVulnerability) {
					...
					msg = String.format(
							"%n%nOne or more dependencies were identified with vulnerabilities that have a CVSS score greater than or equal to '%.1f': "
	 ...
	 */
	protected void showSummary(final MavenProject mp, final Dependency[] dependencies) {
		if (showSummary) {
	...
	/**
	 * Returns the key used to store the path to the data file that is saved by <code>writeDataFile()</code>. This key is used in the
