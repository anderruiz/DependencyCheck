    ...
    public static void showSummary(String projectName, Dependency[] dependencies) {
        ...
        for (Dependency d : dependencies) {
            ...
            if (ids.length() > 0) {
                ...
                List<Identifier> ides = new ArrayList<Identifier>(d.getSoftwareIdentifiers());
                ides.addAll(d.getVulnerableSoftwareIdentifiers());
                StringBuilder sb2 = new StringBuilder();
                for (Identifier i : ides) {
                	if(sb2.length()!=0) {
    					sb2.append(", ");
    				}
    				sb2.append(i.getValue());
				}
                summary.append(sb2);
