...
public class App {
	 ...
	 */
	protected App(final Settings settings) {
	 ...
	 * @throws DatabaseException thrown when there is an error connecting to the database
	 * @throws ExceptionCollection thrown when an exception occurs during analysis; there may be multiple exceptions contained within the
	 * collection.
	 */
	private int runScan(final String reportDirectory, final String outputFormat, final String applicationName, final String[] files,
			final String[] excludes, final int symLinkDepth, final int cvssFailScore)
			throws InvalidScanPathException, DatabaseException, ExceptionCollection, ReportException {
		Engine engine = null;
		try {
	 ...
	 */
	private int determineReturnCode(final Engine engine, final int cvssFailScore) {
		int retCode = 0;
	 ...
	 */
	private Set<File> scanAntStylePaths(final List<String> antStylePaths, final int symLinkDepth, final String[] excludes)
		...
		for (String file : antStylePaths) {
			...
			if (include.startsWith("//")) {
				...
				if (tmpInclude.indexOf('*') >= 0 || tmpInclude.indexOf('?') >= 0 || new File(include).isFile()) {
	 ...
	 */
	private List<String> getPaths(final String[] files) {
	 ...
	 * @throws DatabaseException thrown if a fatal error occurred and a connection to the database could not be established
	 */
	 ...
	 *
	 * @param cli a reference to the CLI Parser that contains the command line arguments used to set the corresponding settings in the core
	 * engine.
	 *
	 * @throws InvalidSettingException thrown when a user defined properties file is unable to be loaded.
	 */
	protected void populateSettings(final CliParser cli) throws InvalidSettingException {
	 ...
	 */
	private void prepareLogger(final String verboseLog) {
	...
	/**
	 * Takes a path and resolves it to be a canonical &amp; absolute path. The caveats are that this method will take an Ant style file
	 * selector path (../someDir/**\/*.jar) and convert it to an absolute/canonical path (at least to the left of the first * or ?).
	 *
	 * @param path the path to canonicalize
	 * @return the canonical path
	 */
	protected String ensureCanonicalPath(final String path) {
		String basePath;
	 ...
	 */
	private int getLastFileSeparator(final String file) {
		if (file.contains("*") || file.contains("?")) {
			int p1 = file.indexOf('*');
			int p2 = file.indexOf('?');
